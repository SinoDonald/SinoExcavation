...
namespace excavation
{
    ...
    class CreateMonitor : IExternalEventHandler
    {
        ...
        public void Execute(UIApplication app)
        {
            ...
            string excel_path = @"\\Mac\Home\Desktop\excavation\filter_data.xlsx";
            ...
            dex.SetData(@"\\Mac\Home\Desktop\excavation\20210615給台大範例\CQ852_20210415.XLS", 4);
            ...
            if (displayStyle.Count() == 0)
            {
                ...
                colorSettings.SetIntermediateColors(new List<AnalysisDisplayColorEntry>
                                                {
                                                    new AnalysisDisplayColorEntry(red, -25),
                                                    new AnalysisDisplayColorEntry(orange, -15),
            ...
            if (sfm == null)
            {
                sfm = SpatialFieldManager.CreateSpatialFieldManager(doc.ActiveView, 2);
            }

            AnalysisResultSchema resultSchema = new AnalysisResultSchema("位移量", "Description");
            resultSchema.SetUnits(new List<string> { "mm" }, new List<double> { 1 });
            int schemaIndex = sfm.RegisterResult(resultSchema);

            try
            {
                int unit_index = 0;
                for (unit_index = 0; unit_index != unit_id_list.Count(); unit_index++)
                {
                    ...
                    sfm.UpdateSpatialFieldPrimitive(idx, new FieldDomainPointsByUV(uvPts), new FieldValues(valList), schemaIndex);
                    ...
                    foreach (GeometryObject geometryObject in geometryElement)
                    {
                        ...
                        if (solid != null)
                        {
                            ...
                            foreach (PlanarFace planarFace in faceArray)
                            {
                                double distance = nearest_visual_coord.DistanceTo(new XYZ(planarFace.Origin.X, planarFace.Origin.Y, 0));
                    ...
                    TaskDialog.Show("1", target_wall.Id.ToString());
                    TaskDialog.Show("1", target_face.Area.ToString());
                    ...
                    List<double> temp_double = sid_displacement.GetRange(sid_index[unit_index], sid_index[unit_index + 1]);
                    for (i = 0; i < n; i++)
                    {
                        valList.Add(new ValueAtPoint(new List<double> { temp_double[i] }));
                    }
                    TaskDialog.Show("1", uvPts.Count().ToString());
                    TaskDialog.Show("1", valList.Count().ToString());
