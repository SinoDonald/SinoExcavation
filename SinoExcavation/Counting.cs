            ...
            try
            {
                ...
                foreach (string file_path in files_path)
                {
                    ...
                    try
                    {
                        ...
                        ste_depth = (double.Parse(Column.LookupParameter("型鋼埋入深度").AsValueString()) / 1000);
                        rad = (double.Parse(Column.LookupParameter("樁徑").AsValueString()) / 1000).ToString();
                    }
                    catch { }
                    ...
                    try
                    {
                        ...
                        try
                        {
                            ...
                            col_unit = Data.Find(reader.column[0].Item5, MatchCase: true);


                            int col_unit_row = col_unit.Row;
                            col_unit_str = DataSheet.Cells[col_unit_row, 14].Value2.ToString();
                        ...
                        IDictionary<string, double> rebar_length_dict = new Dictionary<string, double>();
                        foreach (Rebar r in rebar)
                        {
                            if (!rebar_length_dict.ContainsKey(r.Name.Split(':')[0]))
                            {
                                rebar_length_dict.Add(r.Name.Split(':')[0], 0);
                            }
                        }
                        Excel.Range ERa3 = EWs.Rows["28:" + (28 + rebar_length_dict.Count() - 1).ToString()];
                        ...
                        foreach (Rebar r in rebar)
                        {
                            if (rebar_length_dict.ContainsKey(r.Name.Split(':')[0]))
                            {
                                rebar_length_dict[r.Name.Split(':')[0]] += r.TotalLength;
                            }
                        }
                        int i = 0;
                        foreach (var item in rebar_length_dict)
                        {
                            EWs.Cells[28 + i, 1] = "鋼筋(" + item.Key + ")";
                            EWs.Cells[28 + i, 2] = "m";
                            try
                            {
                                EWs.Cells[28 + i, 4] = (item.Value * 304.8 / 1000).ToString();
                            }
                            catch { }
                            i++;
                        }
