...
namespace SinoExcavation
{
    ...
    class DrawChannelSteel : IExternalEventHandler
    {
        //用哪一個斷面處理
        ...
        public void Execute(UIApplication app)
        {
            ...
            foreach (string file_path in files_path)
            {
                ...
                transaction.Start("create view");
                //取得槽鋼元件
                FamilySymbol channelSteelSymbol = (from x in new FilteredElementCollector(doc).OfClass(typeof(FamilySymbol)).Cast<FamilySymbol>()
                                                   where x.Name == "C200x80"
                                                   select x).ToList().First();

                TaskDialog.Show("Drawing channel steel", "繪製於柱中心連線上");

                if (!channelSteelSymbol.IsActive)
                {
                    channelSteelSymbol.Activate();
                    doc.Regenerate();
                }
                transaction.Commit();
                ...
                try
                {
                    uidoc.PromptForFamilyInstancePlacement(channelSteelSymbol);                    
                }
                catch { };
                ...
                try
                {
                    ...
                    foreach (ElementId famI in familyInstance)
                    {
                        ...
                        if (old_familyInstance.Contains(famI) == false)
                        {
                            ...
                            CopyPaste(famI, sel_combo_xy, sel_single_double, cop, doc, 3, 4);
        ...
        public void CopyPaste(ElementId id, bool XY, bool onetwo, ExReader cop, Document doc, double num, double spaceing)
        {
            //判斷單雙向
            //int i;
            //if (onetwo == true)
            ...
            XYZ origin = ((doc.GetElement(id).Location as LocationCurve).Curve as Line).Origin;

            ElementTransformUtils.MoveElement(doc, id, new XYZ(0, 203.5 / 304.8, 0));
            ...
            for (int one = 1; one != one_slope.Count(); one++)
            {
                ...
                double ne_eleva = (old_math * (-1) + ori_high) * 1000;
                ...
                //偏移元件(Z)
                element.get_Parameter(BuiltInParameter.Z_OFFSET_VALUE).SetValueString(ne_eleva.ToString());

                //鏡像元件
                ElementTransformUtils.MirrorElement(doc, element.Id, Plane.CreateByNormalAndOrigin(XYZ.BasisY, origin));
