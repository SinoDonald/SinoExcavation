            ...
            try
            {
                ...
                try
                {
                    for (int unit_index = 0; unit_index != unit_id_list.Count(); unit_index++)
                    {
                        ...
                        //curves.Append(Line.CreateBound(x1, x2));
                        //curves.Append(Line.CreateBound(x2, x3));
                        //curves.Append(Line.CreateBound(x3, x4));
                        //curves.Append(Line.CreateBound(x4, x1));
                        //CurveLoop curveLoop = CurveLoop.Create(curves);
                        //IList<CurveLoop> curveLoops = new List<CurveLoop>();
                        //curveLoops.Add(curveLoop);
                        //Element floor = Floor.Create(doc, curveLoops, floorType.Id, Level.Create(doc, 0).Id);
